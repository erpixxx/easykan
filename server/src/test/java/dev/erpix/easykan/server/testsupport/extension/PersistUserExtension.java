...
public class PersistUserExtension implements AfterEachCallback, BeforeEachCallback {
    ...
    public void afterEach(ExtensionContext context) {
        ...
        CacheManager cacheManager =
                SpringExtension.getApplicationContext(context).getBean(CacheManager.class);
    ...
    private User createPersistedUser(ExtensionContext context, WithPersistedUser ann) {
        ...
        User user = User.builder().login(ann.login()).displayName(ann.displayName())
                .email(ann.email()).passwordHash(passwordEncoder.encode(ann.password()))
                .permissions(UserPermission.toValue(ann.permissions())).build();
