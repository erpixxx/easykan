...
import dev.erpix.easykan.server.testsupport.security.WithPersistedUsers;
...
public class UserServiceIT {
    ...
    @WithPersistedUsers(principal = @WithPersistedUser(
            login = "admin",
            email = "admin@easykan.dev",
            permissions = UserPermission.MANAGE_USERS
    ), others = {
            @WithPersistedUser(login = "user1", email = "user1@easykan.dev"),
    ...
    void createUser_shouldCreateNewUser_whenUserHasPermission() {
        ...
        assertThat(createdUser.getLogin())
                .isEqualTo(requestDto.login());
        assertThat(createdUser.getDisplayName())
                .isEqualTo(requestDto.displayName());
        assertThat(createdUser.getEmail())
                .isEqualTo(requestDto.email());
    ...
    @WithPersistedUsers(principal = @WithPersistedUser(
            ...
            email = "admin@easykan.dev",
            permissions = UserPermission.MANAGE_USERS
    ), others = @WithPersistedUser(
            login = "usertodelete",
            email = "usertodelete@easykan.dev"
    ))
    void deleteUser_shouldDeleteUser_whenUserExistsAndHasPermission() {
        ...
        assertThat(userRepository.existsById(userToDelete.getId()))
                .isFalse();
        assertThat(userRepository.count())
                .isEqualTo(1);
    }
    ...
    void updateCurrentUserInfo_shouldUpdateUserInfo_whenRequestIsValid() {
        ...
        assertThat(userFromDb.getDisplayName())
    ...
    @WithPersistedUsers(principal = @WithPersistedUser(
            ...
            login = "usertoupdate",
            email = "usertoupdate@easykan.dev"
    ))
    void updateUserInfo_shouldUpdateUserInfo_whenRequestIsValidAndUserHasPermissions() {
        User userToUpdate = userRepository.findByLogin("usertoupdate").orElseThrow(() ->
                new AssertionError("User to update not found"));
    ...
    void updateUserPermissions_shouldUpdateUserPermissions_whenUserHasAdminPermission() {
        ...
        assertThat(UserPermission.hasPermission(updatedUser, UserPermission.MANAGE_USERS))
                .isTrue();
    }
