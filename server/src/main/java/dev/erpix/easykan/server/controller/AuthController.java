...
public class AuthController {
	...
	@Operation(summary = "Log in with password", description = "Authenticates a user with login and password.")
	...
	public ResponseEntity<UserResponseDto> login(@RequestBody AuthLoginRequestDto request) {
		...
		return ResponseEntity.ok()
			.header(HttpHeaders.SET_COOKIE,
					createCookie("access_token", tokenPairDto.newAccessToken(), tokenPairDto.newAccessTokenDuration()))
			.header(HttpHeaders.SET_COOKIE,
					createCookie("refresh_token", tokenPairDto.newRawRefreshToken(),
							tokenPairDto.newRefreshTokenDuration()))
			.body(user);
	}
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successful logout from all sessions"),
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Tokens refreshed successfully"),
			@ApiResponse(responseCode = "401", description = "Invalid or missing refresh token") })
	@PostMapping("/refresh")
