...
public record UserCreateRequestDto(@Login String login,@DisplayName String displayName,@Email String email,@Password String password){

public @NotNull User toUser(){return User.builder().login(login).displayName(displayName).email(email).passwordHash(password).build();}}
