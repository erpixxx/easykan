...
public abstract class LoginConstraintValidator<T> implements StringConstraintValidator<Login, T> {
	...
	public boolean isStringContentValid(String value, ConstraintValidatorContext context) {
		...
		if (value.isBlank()) {
			context.buildConstraintViolationWithTemplate("Login cannot be blank").addConstraintViolation();
			return false;
		}
		...
		if (value.length() < ValidationConstants.LOGIN_MIN_LENGTH
			...
			context
				.buildConstraintViolationWithTemplate("Login must be between" + ValidationConstants.LOGIN_MIN_LENGTH
						+ " and " + ValidationConstants.LOGIN_MAX_LENGTH + " characters long")
